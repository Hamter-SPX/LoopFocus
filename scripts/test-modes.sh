...
echo "$out" | grep -q '"mode":"zero-debug-state"' || { echo "T1 FAIL: fix bug should resolve to zero-debug-state"; fail=1; }
...
node "$S/mode.js" show zero-debug-state | grep -q '"closes_when"' || { echo "T5 FAIL: show should print full contract"; fail=1; }

mkdir -p "$TMP/.loopfocus"
printf '{"mode":"zero-debug-state","goal_locked":true,"gates_ran":["entry","context","mutation","build","test","regression","progress","repeat","completion"],"self_audit_pass":true}' > "$TMP/.loopfocus/mode-state.json"
(cd "$TMP" && node "$S/mode.js" check zero-debug-state) | grep -q '"verdict":"PASS"' || { echo "T6 FAIL: complete state should pass check"; fail=1; }

printf '{"mode":"zero-debug-state","goal_locked":false,"gates_ran":[],"self_audit_pass":false}' > "$TMP/.loopfocus/mode-state.json"
(cd "$TMP" && node "$S/mode.js" check zero-debug-state) | grep -q '"verdict":"FAIL"' || { echo "T7 FAIL: incomplete state should fail check"; fail=1; }
